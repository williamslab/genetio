...
uint32_t countBitsSet(chunk value) {
  // in K&R:
  uint32_t count; // c accumulates the total bits set in value
...
uint32_t countBitsSetDense(chunk value) {
  uint32_t count = 0; // c accumulates the total bits set in value
  while (value > 0) {
...
uint32_t countHighOrderUnsetBits(chunk value) {
  uint32_t highestBitIdx = getHighestOrderBitIdx(value);
...
int chunkHashFunc(const chunk &key) {
  ...
  for(uint32_t i = 0; i < (sizeof(chunk) / 2); i++) {
...
//
//  uint64_t totalClustersOverChunks = 0; // for genome-wide average
//  int numHapChunks = Marker::getNumHapChunks();
//  for(int curChunk = 0; curChunk < numHapChunks; curChunk++) {
//
//    printf("Chunk %5d: ", curChunk);
